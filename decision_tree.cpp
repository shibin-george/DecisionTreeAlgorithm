...
public:
    ...
    {
        tuple_class = values[attributes];
    }
};

db_tuple database[1024];
dec_node *root = NULL;
...
bool check_database(int index[], int siz)
{
    bool proceed = false;
    map<string ,bool> c;
    c[database[index[0]].values[attributes-1]] = true;
    for(int i=1; i<siz; i++)
    {
        string s = database[index[i]].values[attributes-1];
        if(c[s] == false)
...
{
    bool proceed = check_database(index, siz);
    if(proceed)
    {
        vector<string> val(1);
        val.resize(1000);
